...
* Run the PostgreSQL database server (using docker: `docker run  -e POSTGRES_PASSWORD=secrect -e POSTGRES_USER=postgres -d -t -i -p 5432:5432 postgres`)
* Run the server with `uvicorn billtitles.main:app --reload` (default port `8000`)

To set up pyenv with python 3.9.1 in this case, do the following:

* pyenv install 3.9.1
* pyenv virtualenv 3.9.1 billtitlesenv
* pyenv activate billtitlesenv
...
The database container for postgres if one wants to manually generate all the tables can be done via `docker run  --name billsim-data  -p 5432:5432 -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=postgres -d postgres`
...
NOTE: If you run it this way, replace the postgres_url with
`postgresql://postgres:postgres@localhost:5432`

NOTE: The order running then would be to set up the table and then run this,
