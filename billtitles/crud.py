...
class BillsResponse(TypedDict):
    bills: List[billtitles.schemas.BillTitlePlus]
    bills_title_whole: List[billtitles.schemas.BillTitlePlus]
...
def get_bill_titles_by_billnumber(db: Session, billnumber: str = None):
    ...
    titles_whole_resp = (
        db.query(
            models.Bill,
            func.string_agg(models.Title.title, literal_column("'; '")).label("titles"),
        )
        .filter(models.Bill.billnumber == billnumber)
        .join(models.BillTitle, models.BillTitle.bill_id == models.Bill.id)
        .join(models.Title, models.Title.id == models.BillTitle.title_id)
        .filter(models.BillTitle.is_for_whole_bill == True)
        .group_by(models.Bill.id)
        .all()
    )
    titles_all_resp = (
        db.query(
    ...
    if len(titles_all_resp) > 0:
        ...
        titles_all = []
    return billtitles.schemas.BillTitleResponse(
        billnumber=billnumber,
        titles=billtitles.schemas.TitlesItem(whole=titles_whole, all=titles_all),
    )
...
def get_related_bills(
...
) -> List[billtitles.schemas.BillToBillModel or billtitles.schemas.BillToBillModelDeep]:
    if not billnumber:
        return []
    billnumber = billnumber.strip("\"'").lower()
    if version:
    ...
    if not version:
        subquery = (
            db.query(
                models.Bill.billnumber,
                models.Bill.version,
                models.Bill.length,
                models.BillToBill.score,
                models.BillToBill.score_to,
                models.BillToBill.reasonsstring,
                models.BillToBill.sections_num,
                models.BillToBill.sections_match,
                models.BillToBill.score_es,
                models.BillToBill.bill_id,
                models.BillToBill.bill_to_id,
            )
            .filter(models.Bill.billnumber == billnumber)
            .join(models.BillToBill, models.BillToBill.bill_id == models.Bill.id)
            .subquery()
        )
        bills = (
            db.query(
                bill_to.billnumber.label("billnumber_to"),
                bill_to.version.label("version_to"),
                bill_to.length.label("length_to"),
                subquery,
            )
            .filter(subquery.c.bill_to_id == bill_to.id)
        ...
        subquery = (
            ...
            .filter(
                models.Bill.billnumber == billnumber,
                models.Bill.version == version,
            )
            .join(models.BillToBill, models.BillToBill.bill_id == models.Bill.id)
            .subquery()
        )
    ...
    for bill in bills:
        bill = bill._asdict()
        bill["billnumber_version"] = bill.get("billnumber", "") + bill.get(
            "version", ""
        )
        bill["length_to"] = bill.get("length_to", 0)
        bill["billnumber_version_to"] = bill.get("billnumber_to", "") + bill.get(
            "version_to", ""
        )
    ...
    if flat == False:
        ...
        for billdict in billdicts:
            ...
            billdict_deep_to = billtitles.schemas.BillModelDeep(
                **{
                    keyitem.replace("_to", ""): billdict[keyitem]
                    for keyitem in billdict.keys()
                    if keyitem.find("_to") > -1 and keyitem not in extrafields
                },
                **billdict_deep_extra
            )
...
def get_bills(db: Session, skip: int = 0, limit: int = 100):
    return (
        db.query(models.BillTitle.billnumber, models.BillTitle.title)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_title_by_id(db: Session, title_id: int):
    return (
        db.query(models.BillTitle).filter(models.BillTitle.title_id == title_id).first()
    )


def get_title(db: Session, title: str) -> billtitles.schemas.TitleBillsResponse:
    title = title.strip("\"'")

    titles = [
        billtitles.schemas.TitleBillsResponseItem(
            id=item.id,
            title=item.title,
            billnumbers=item.billnumbers.split("; "),
        )
        for item in db.query(
            models.Bill.billnumber,
            models.Title.title,
            models.BillTitle.title_id,
            func.group_concat(models.Bill.billnumber, "; ").label("billnumbers"),
        )
        .filter(models.Title.title == title)
        .group_by(models.Title.title)
        .all()
    ]
    titles_whole = [
        billtitles.schemas.TitleBillsResponseItem(
    ...
    return billtitles.schemas.TitleBillsResponse(
        titles=titles, titles_whole=titles_whole
    )


def get_titles(db: Session, skip: int = 0, limit: int = 100):
    return (
        db.query(models.Bill.billnumber, models.Title.title)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_titles_by_bill_id(db: Session, bill_id: int):
    return (
        db.query(models.BillTitle, models.Title.title)
        .filter(models.BillTitle.bill_id == bill_id)
        .join(models.Title, models.Title.id == models.BillTitle.title_id)
        .all()
    )
...
def get_title_by_billnumber(db: Session, billnumber: str = None):
    ...
    titles = (
        db.query(
            models.Bill.billnumber,
            func.group_concat(models.Title.title, "; ").label("titles"),
        )
        .filter(models.Bill.billnumber == billnumber)
        .group_by(models.Bill.billnumber)
        .all()
    )
    titles_whole = (
        db.query(
...
def get_billtitle(db: Session, title: str, billnumber: str):
    return (
        db.query(models.Bill.billnumber, models.Title.title)
        .filter_by(title=title, billnumber=billnumber)
        .first()
    )
